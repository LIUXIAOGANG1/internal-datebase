	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for (String key : request.getParameterMap().keySet()) {
			if(logger.isDebugEnabled()){
				logger.debug("[{}, {}]", key, Arrays.toString(request.getParameterMap().get(key)));
			}
		...
		if (!isSet(request.getParameter("Confirm")) && isSet(request.getParameter("checkout"))) {
			...
			if (isSet(session.getAttribute("EXPRESS_MARK")) && session.getAttribute("EXPRESS_MARK").equals("ECMark")) {
				if (isSet(request.getParameter("shipping_method"))) {
					BigDecimal new_shipping = new BigDecimal(request.getParameter("shipping_method")); 
			...
			if (strAck != null && (strAck.equals("SUCCESS") || strAck.equals("SUCCESSWITHWARNING"))) {
				...
				String errorString = "SetExpressCheckout API call failed. " + "<br>Detailed Error Message: " + ErrorLongMsg + "<br>Short Error Message: " + ErrorShortMsg + "<br>Error Code: " + ErrorCode + "<br>Error Severity Code: " + ErrorSeverityCode;
